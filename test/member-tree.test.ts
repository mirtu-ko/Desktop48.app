...
import { addMemberFlagId, hasMemberFlagId, removeMemberFlagId, resolveMemberFlags } from '../src/main/domain/member-flags'
...
describe('member-flags 纯函数', () => {
  const starInfo: MemberRecord[] = [
    { userId: 1, realName: '甲' },
    { userId: 2, realName: '乙' },
  ]
  it('addMemberFlagId 幂等，已存在返回 null', () => {
    expect(addMemberFlagId([], 1)).toEqual([1])
    expect(addMemberFlagId([1], 1)).toBeNull()
    expect(addMemberFlagId(undefined, 3)).toEqual([3])
    const list = [1]
    addMemberFlagId(list, 2)
    expect(list).toEqual([1])
  })
  it('hasMemberFlagId / removeMemberFlagId 做数值归一化（兼容历史字符串 id）', () => {
    expect(hasMemberFlagId(['1', 2], 1)).toBe(true)
    expect(removeMemberFlagId(['1', 2], 1)).toEqual([2])
    expect(removeMemberFlagId(undefined, 1)).toEqual([])
  })
  it('resolveMemberFlags 跳过查不到的 id，成员表缺失时为空', () => {
    expect(resolveMemberFlags([1, 999], starInfo)).toEqual([starInfo[0]])
    expect(resolveMemberFlags(undefined, starInfo)).toEqual([])
    expect(resolveMemberFlags([1], undefined)).toEqual([])
  })
})
