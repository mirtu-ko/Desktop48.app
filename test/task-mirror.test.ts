 ...
 * 是独立播放窗与主窗口列表保持一致的关键，但纯几何测试覆盖不到，故单独成文件。
 */
...
describe('decideTaskMerge', () => {
  it('本窗口发起：列表已有且在跟踪 → skip（不重复建卡片、不重复弹提示）', () => {
    ...
    expect(decideTaskMerge(list, runningSnapshot)).toBe('skip')
  })
  ...
  it('本窗口已结束的任务被重启：有卡片但监听器已清空 → resync（按快照重新对齐）', () => {
    ...
    expect(decideTaskMerge(list, runningSnapshot)).toBe('resync')
  })

  it('其它窗口发起：列表没有该 liveId → mirror（新建镜像卡片）', () => {
    expect(decideTaskMerge([makeTask('other', 2)], runningSnapshot)).toBe('mirror')
    expect(decideTaskMerge([], runningSnapshot)).toBe('mirror')
  })

  it('快照非 running 时不做 resync（已结束的任务无需补监听器）', () => {
    const list = [makeTask('live-1', 0)]
    const finished = { ...runningSnapshot, status: 'finish' as const }
    expect(decideTaskMerge(list, finished)).toBe('skip')
  })
