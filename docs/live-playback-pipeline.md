...
| 缩放     | 系统边框自由缩放；`@aspect` 到达时**按工作区整体重算**（不是只保宽度），并保持右上角不动。**首次定形无条件执行** —— 建窗用的是 9:16 兜底比例，此时「窗口实际尺寸 ≠ 请求值」是常态（Windows per-monitor DPI、Linux WM 取整），据此判定「用户缩放过」会让首次定形被整个跳过，横屏视频就一直停在竖屏兜底的窄窗上；定形落定后才尊重用户缩放、不再自动改尺寸（判据 `decideAutoFit`）                                                                                                                                                                                                                                                             |
| 最大化   | 标题栏自带最大化 / 还原按钮（`maximizable: true`），双击标题栏同效 —— 是**系统级**最大化，与已移除的 DOM 浮层「放大态」无关。状态由主进程推 `windowOnMaximizeChange`（`sendIpc` 而非广播：各窗口状态彼此独立），渲染端据此换图标；最大化视同「用户手动接管尺寸」（置位 `userResized`），后续 `@aspect` 上报不会把它抹掉                                                                                                                                                                                                                                                                                                                     |
| 初始尺寸 | **按所在显示器工作区比例自适应**，不写死像素：视频区宽 = 工作区宽 × 0.45（横屏的决定项）、高上限 = 工作区高 × 0.7（竖屏的决定项），按视频比例内接后叠加标题栏（`computeInitialWindowSize`）。两个比例各管一个方向，横竖屏互不干扰；`workArea` 是 DIP，故 4K@200% 与 1080p@100% 观感一致。**电台窗例外**：给固定竖屏尺寸 `FLOAT_RADIO_WINDOW_SIZE`（360×640，含标题栏），**不参与**工作区比例推导 —— 电台没有视频轨、永不上报 aspect，形状不需要跟着画面走，为它做自适应收益为零。直播与回放同一判据（`payload.liveType === 2`）—— 回放的 `liveType` 由 `Playbacks.vue` 从列表项透传，不能等 `PlaybackPlayer` 拉完详情接口，那时窗口早建好了 |
...
⚠️ 窗口控制通道（`windowClose` / `windowMinimize` / `windowToggleMaximize` 等）一律以 `BrowserWindow.fromWebContents(event.sender)`
...
- `will-resize` / `resized` 事件 **只有 macOS / Windows 触发**，Linux 不发射。因此「用户手动缩放过」不能只靠
  ...
  该兜底**只在首次定形落定之后生效** —— 否则建窗时的 DPI / WM 偏差会被误判成用户缩放，把首次定形整个跳过
  （横屏视频停在竖屏窄窗，且无任何报错）。
